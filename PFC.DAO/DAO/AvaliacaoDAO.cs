    ...
    public class AvaliacaoDAO
    {
        ...
        public async Task<Avaliacao> InserirPontoLikeDeslike(Avaliacao avaliar, double pontosUsuario)
        {
            ...
            using (contexto = new Contexto())
            {
                ...
                string strQuery = String.Format("insert into Avaliacao (Id_Topico,Id_Usuario,Data_Avaliacao,Pontos,id_Usuario_DonoTopico) Values({0},{1},(Select GETDATE()),{2},(Select Id_Usuario from Topico where Id = {0})) select Pontos,(select count(Pontos) from Avaliacao where Id_Topico = {0} and Pontos = 1) as contarLike,(select count(Pontos) from Avaliacao where Id_Topico = {0} and Pontos = -1) as contarDeslike from Avaliacao where Id_Topico = {0} and Id_Usuario = {1}", avaliar.idTopico, avaliar.idUsuario, pontosUsuario);
