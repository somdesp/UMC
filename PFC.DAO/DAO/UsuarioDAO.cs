    ...
    public class UsuarioDAO
    {
        ...
        public bool AdicionarUsuario(Usuario usuario)
        {
            ...
            strQuery += string.Format("VALUES('{0}','{1}','{2}',PWDENCRYPT('{3}'),'{4}','{5}','{6}','{7}','{8}','{9}','{10}',(select max(Id) from Arquivos))",
                usuario.Nome, usuario.Login, usuario.Email, usuario.Senha, usuario.Sexo.Id, usuario.DataNasci.ToString("yyyy-MM-dd"), usuario.RGM, usuario.Curso.Id, usuario.Semestre.Id, DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"), 2);
